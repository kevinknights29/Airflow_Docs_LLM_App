...
from hypothesis.stateful import MultipleResults
...
dtype_shared_st: st.SearchStrategy[np.float16 | np.float32 | np.float64] = st.shared(
    st.sampled_from(strategies.float_types),
    key="dtype",
)
