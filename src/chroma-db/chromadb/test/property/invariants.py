...
from chromadb.test.property.strategies import NormalizedRecordSet
...
def wrap(value: T | list[T]) -> list[T]:
...
def wrap_all(record_set: RecordSet) -> NormalizedRecordSet:
    ...
    embedding_list: types.Embeddings | None
...
def _field_matches(
    ...
    field_name: Literal["documents"] | Literal["metadatas"],
) -> None:
    """
...
def ann_accuracy(
    ...
    embeddings: types.Embeddings | None = normalized_record_set["embeddings"]
