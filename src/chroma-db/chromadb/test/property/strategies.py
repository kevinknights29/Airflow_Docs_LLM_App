...
class RecordSet(TypedDict):
    ...
    ids: types.ID | list[types.ID]
    embeddings: types.Embeddings | types.Embedding | None
    metadatas: list[types.Metadata] | types.Metadata | None
    documents: list[types.Document] | types.Document | None
...
class NormalizedRecordSet(TypedDict):
    ...
    ids: list[types.ID]
    embeddings: types.Embeddings | None
    metadatas: list[types.Metadata] | None
    documents: list[types.Document] | None
...
class Record(TypedDict):
    ...
    id: types.ID
    embedding: types.Embedding | None
    metadata: types.Metadata | None
...
safe_values: list[SearchStrategy[int | float | str]] = [
    safe_text,
    safe_integers,
    safe_floats,
]
...
def one_or_both(
    strategy_a: st.SearchStrategy[Any],
    strategy_b: st.SearchStrategy[Any],
...
class Collection:
    ...
    embedding_function: types.EmbeddingFunction | None = None
...
def collections(
    ...
    known_metadata_keys: dict[str, int | str | float] = {}
...
def metadata(draw: st.DrawFn, collection: Collection) -> types.Metadata:
    ...
    if collection.known_metadata_keys:
        ...
        sampling_dict: dict[str, st.SearchStrategy[str | int | float]] = {
...
def recordsets(
    ...
    if len(ids) == 1:
        single_id: str | list[str] = ids[0] if draw(st.booleans()) else ids
        single_embedding = (
            embeddings[0]
            if embeddings is not None and draw(st.booleans())
            else embeddings
        )
        single_metadata: Metadata | list[Metadata] = (
...
def where_clause(draw: st.DrawFn, collection: Collection) -> types.Where:
    ...
    legal_ops: list[str | None] = [None, "$eq", "$ne"]
...
class Filter(TypedDict):
    where: types.Where | None
    ids: str | list[str] | None
    where_document: types.WhereDocument | None
...
def filters(
    ...
    ids: list[types.ID] | types.ID | None
